...
class FALI_subscriber {
	...
	*/
		...
		if ( 'subscrib' == esc_attr( $_REQUEST['subscrib_key'] ) ) {
			...
			if ( is_null( $row ) ):
				...
				if ( $insert ) {
					...
					wp_send_json( array( 'type' => "Insert", "message" => 'Error' ) );
				}
	...
	private function sendMail( $mail, $post_id ) {
		...
		$body .= '<h1 style="text-align:center">' . $post->post_title . '</h1> <br> 
' . $body . ' <div> </div> <div><em>Suivre le lien: <a href="' . get_permalink( $post ) . '">' . $post->post_title . '</a></em></div>';
